...
grid tx gov submit-legacy-proposal param-change proposal.json --from=<key-name> --fees=0.3grid --chain-id=gridfury
```
...
grid tx gov submit-legacy-proposal community-pool-spend proposal.json --from=<key-name> --fees=0.3grid --chain-id=gridfury
```
