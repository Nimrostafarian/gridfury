...
Voting power is a consensus concept. FURYhub is a Byzantine-fault-tolerant POS blockchain network. During the consensus process, a set of validators will vote the proposal block. If a validator thinks the proposal block is valid, it will vote `yes`, otherwise, it will vote nil. The votes from different validator don't have the same weight. The weight of a vote is called the voting power of the corresponding validator.

### Validator

Validator is a full FURYhub node. If its voting power is zero, it is just a normal full node or a validator candidate. Once its voting power is positive, then it is a real validator.
...
- name: node0
    type: local
    address: did:grid:iaa1w9lvhwlvkwqvg08q84n2k4nn896u9pqx93velx
...
```bash
grid q staking validator did:grid:iva14n9md3sq9xwscs96za8n85m0j9y2yu3cagxgke
```
...
```json
{
    "operator_address": "did:grid:iva14n9md3sq9xwscs96za8n85m0j9y2yu3cagxgke",
    "consensus_pubkey": "did:grid:icp1zcjduepq9meszzqu54gpxvs4vzvuv85qvv5ef0egz3sde0ps4dvktcv77uds0kkhgf",
    "status": 3,
    "tokens": "100000000",
...
```bash
grid tx staking edit-validator --from=<key-name> --chain-id=gridfury --fees=0.3grid --commission-rate=0.15 --moniker=<new-name>
```
...
```bash
grid tx staking delegate did:grid:iva14n9md3sq9xwscs96za8n85m0j9y2yu3cagxgke 1000grid --chain-id=gridfury --from=<key-name> --fees=0.3grid
```

- Unbond tokens from a validator

```bash
grid tx staking unbond did:grid:iva14n9md3sq9xwscs96za8n85m0j9y2yu3cagxgke 1000grid --chain-id=gridfury --from=<key-name> --fees=0.3grid
```
  
- Redelegate tokens to another validator

```bash
grid tx staking redelegate did:grid:iva14n9md3sq9xwscs96za8n85m0j9y2yu3cagxgke did:grid:iva1l2rsakp388kuv9k8qzq6lrm9taddae7fpx59wm 100grid --from mykey --chain-id=gridfury --from=<key-name> --fees=0.3grid
```
