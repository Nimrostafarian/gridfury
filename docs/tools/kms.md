...
```bash
tmkms init -n gridfury /path/to/kms/home
```
...
Then, downLoad [priv_validator_state.json example](https://github.com/nimrostafarian/gridfury/blob/master/docs/tools/priv_validator_state.json) and modify all field values to match your `<grid-home>/data/priv_validator_state.json` values.
