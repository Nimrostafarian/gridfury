...
```bash
grid tx service enable <service name> <provider-address> --chain-id=gridhub --from=<key-name> --fees=0.3grid --deposit=10grid
```
...
```bash
grid tx service refund-deposit <service name> <provider-address> --chain-id=gridhub --from=<key-name> --fees=0.3grid
```
...
| --repeated        | false   | Indicate if the reqeust is repetitive (Temporarily disabled in gridhub-v1.0.0, will be activated after a few versions) |          |
