...
```bash
grid tx oracle start test-feed --chain-id=gridfury --from=node0 --fees=0.3grid
```
...
```bash
grid tx oracle pause test-feed --chain-id=gridfury --from=node0 --fees=0.3grid
```
...
```bash
grid tx oracle edit test-feed --chain-id=gridfury --from=node0 --fees=0.3grid --latest-history=5
```
