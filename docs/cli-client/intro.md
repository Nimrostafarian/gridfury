...
`grid` is a command line client for the FURYhub network. FURYhub users can use `grid` to send transactions and query the blockchain data.

## Working Directory

The default working directory for the `grid` is `$HOME/.grid`, which is mainly used to save configuration files and data. The FURYhub `key` data is saved in the working directory of `grid`. You can also specify the `grid`  working directory by `--home`.
