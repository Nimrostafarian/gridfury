...
```bash
grid tx staking create-validator --chain-id=gridhub --from=<key-name> --fees=0.3grid --pubkey=<validator-pubKey> --commission-rate=0.1 --amount=100grid --moniker=<validator-name>
```
...
```bash
grid tx staking edit-validator --from=<key-name> --chain-id=gridhub --fees=0.3grid --commission-rate=0.10 --moniker=<validator-name>
```
