...
Example:
	grid testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "grid", "Home directory of the node's daemon configuration")
...
func InitTestnet(
	...
	for i := 0; i < numValidators; i++ {
		...
		coins := sdk.Coins{
			...
			sdk.NewCoin(gridtypes.EvmToken.MinUnit, accEvmTokens),
		...
		customAppTemplate, _ := servercfg.AppConfig(gridtypes.NativeToken.MinUnit)
...
func initGenFiles(
	...
	tokenGenState.Tokens = append(tokenGenState.Tokens, gridtypes.EvmToken)
	...
	serviceGenState.Bindings = append(serviceGenState.Bindings, servicetypes.GenOraclePriceSvcBinding(gridtypes.NativeToken.MinUnit))
	...
	evmGenState.Params.EvmDenom = gridtypes.EvmToken.MinUnit
