...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=grid \
		  ...
		  -X github.com/nimrostafarian/gridfury/types.EIP155ChainID=4200 \
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./lite/statik/statik.go" -not -path "*.pb.go" | xargs goimports -w -local github.com/nimrostafarian/gridfury
...
testnet-init:
	@if ! [ -f build/nodecluster/node0/grid/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/home nimrostafarian/gridfury grid testnet --v 4 --output-dir /home/nodecluster --chain-id gridfury-test --keyring-backend test --starting-ip-address 192.168.10.2 ; fi
	@echo "To install jq command, please refer to this page: https://stedolan.github.io/jq/download/"
	@jq '.app_state.auth.accounts+= [{"@type":"/cosmos.auth.v1beta1.BaseAccount","address":"did:grid:iaa1ljemm0yznz58qxxs8xyak7fashcfxf5lgl4zjx","pub_key":null,"account_number":"0","sequence":"0"}] | .app_state.bank.balances+= [{"address":"did:grid:iaa1ljemm0yznz58qxxs8xyak7fashcfxf5lgl4zjx","coins":[{"denom":"ugrid","amount":"1000000000000"}]}]' build/nodecluster/node0/grid/config/genesis.json > build/genesis_temp.json ;
	...
	@echo "Faucet address: did:grid:iaa1ljemm0yznz58qxxs8xyak7fashcfxf5lgl4zjx" ;
